...
const HeroFeaturedProject = ({ project, theme }) => {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 md:px-8 mb-16 md:mb-28">
      <Reveal>
        <div className="relative w-full rounded-[28px] md:rounded-[36px] border border-white/10 bg-[#0B0D14]/80 backdrop-blur-2xl p-5 md:p-10 overflow-hidden group shadow-[0_30px_70px_rgba(0,0,0,0.5)]">
          {/* Subtle Backglow */}
          <div 
          ...
          <div className="flex flex-col lg:flex-row gap-6 md:gap-8 lg:gap-12 items-center relative z-10">
            ...
            <div className="w-full lg:w-1/2 flex flex-col justify-between h-full align-stretch">
              <div>
                <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-3 md:mb-4">
                  <span className="rounded-full border border-amber-500/30 bg-amber-500/10 px-2.5 py-0.5 md:px-3 md:py-1 text-[8px] md:text-[10px] font-bold uppercase tracking-[0.25em] text-amber-400">
                    Featured Experience
                  </span>
                  <span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-0.5 md:px-3 md:py-1 text-[8px] md:text-[10px] uppercase tracking-[0.2em] text-white/60">
                    {project.category || "Case Study"}
                  </span>
                </div>

                <h3 className="text-[22px] sm:text-[28px] md:text-[42px] font-bold tracking-tight text-white leading-tight mb-3 md:mb-4">
                  {project.title}
                </h3>

                <p className="text-[13px] md:text-[16px] leading-relaxed text-[#94A3B8] font-medium mb-4 md:mb-6">
                  {project.desc}
                </p>

                {/* Metrics Matrix */}
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4 border-t border-b border-white/5 py-4 md:py-5 mb-5 md:mb-6">
                  <div>
                    <span className="text-[9px] md:text-[10px] uppercase tracking-wider text-white/40 block mb-0.5 md:mb-1">Industry</span>
                    <span className="text-[12px] md:text-[13px] font-semibold text-[#F1F5F9]">{project.industry || "Digital Solutions"}</span>
                  </div>
                  <div>
                    <span className="text-[9px] md:text-[10px] uppercase tracking-wider text-white/40 block mb-0.5 md:mb-1">Core Focus</span>
                    <span className="text-[12px] md:text-[13px] font-semibold text-[#F1F5F9]">{project.goal || "Premium Experience"}</span>
                  </div>
                  <div>
                    <span className="text-[9px] md:text-[10px] uppercase tracking-wider text-amber-400 block mb-0.5 md:mb-1">Business Impact</span>
                    <span className="text-[12px] md:text-[13px] font-bold text-amber-400">{project.result || "Conversion Optimized"}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between mt-auto pt-1">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 rounded-full bg-white px-5 py-2.5 md:px-6 md:py-3 text-[11px] md:text-[12px] font-bold uppercase tracking-[0.14em] text-black shadow-[0_10px_30px_rgba(255,255,255,0.15)] transition hover:bg-neutral-200 hover:scale-[1.02]"
                >
                  Explore Experience <Plus size={13} className="text-black" />
                </a>
                ...
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="flex h-10 w-10 md:h-12 md:w-12 items-center justify-center rounded-full border border-white/10 bg-[#16171D] text-[#94A3B8] transition duration-300 hover:text-white hover:border-white/30"
                >
          ...
          {items.map((item, index) => (
            <Reveal key={item._id} delay={index * 0.1}>
              <motion.a
                ...
                className="group relative h-auto min-h-[490px] w-[85vw] sm:w-[350px] md:h-[590px] md:w-[370px] lg:w-[385px] shrink-0 snap-center md:snap-start overflow-hidden rounded-[24px] md:rounded-[28px] border border-cyan-500/10 bg-[#0B0D14] shadow-[0_20px_50px_rgba(56,189,248,0.15)] backdrop-blur-xl flex flex-col p-3.5 md:p-4"
              >
                ...
                <div className="relative h-[180px] md:h-[45%] w-full bg-[#141622] rounded-[18px] md:rounded-[20px] border border-white/5 p-2 flex items-center justify-center overflow-hidden transition duration-500 group-hover:border-cyan-500/20 shrink-0">
                  ...
                  {/* METRIC CATEGORIES INSTEAD OF REPETITIVE LABELS */}
...
function Projects() {
  ...
  // ✅ FIX: Filter out the Hero project from the row carousels to prevent repetition (Image 2 Fix)
  const filterHero = (items = []) => items.filter(p => p._id !== featuredHeroProject?._id);
  ...
  return (
    <section id="projects" className="relative overflow-hidden bg-transparent py-12 md:py-24 text-white">
      <div className="mx-auto max-w-7xl">
        ...
        {/* SLIDER ROW CATEGORIES - Dynamically filtered to remove duplicates */}
        <ProjectSection title="Development" items={filterHero(projects.web)} theme={PROJECT_THEMES.web} />
        <ProjectSection title="UI / UX Design" items={filterHero(projects.uiux)} theme={PROJECT_THEMES.uiux} />
        <ProjectSection title="Motion & Editing" items={filterHero(projects.editing)} theme={PROJECT_THEMES.editing} />
      </div>
    </section>
  );
}
