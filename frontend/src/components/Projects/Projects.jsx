...
const ProjectCard = ({ item, theme, offset, isActive, onDragEnd }) => {
  ...
  return (
    <motion.div
      ...
      initial={false}
      animate={{ x: `${translateX}%`, z: translateZ, rotateY, opacity, scale: isActive ? 1 : 0.80 }}
      ...
      style={{ boxShadow: isActive ? theme.shadow.boxShadow : "0 20px 50px rgba(0,0,0,0.7)" }}
    >
      ...
      <div className="w-full md:w-[52%] lg:w-1/2 flex flex-col justify-between h-full py-1">
        <div>
          <div className="flex items-center gap-2 mb-3 md:mb-4">
            <span className={`rounded-full bg-gradient-to-r ${theme.color} p-[1px]`}>
              <span className="block rounded-full bg-[#0A0B10] px-3 py-0.5 text-[8px] md:text-[9px] font-black uppercase tracking-[0.25em] text-white">Featured Experience</span>
            </span>
        ...
        <div className="flex items-center justify-between mt-auto pt-1 pointer-events-auto">
          <a href={item.url || "#"} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className={`inline-flex items-center gap-2 rounded-full bg-gradient-to-r ${theme.color} px-4 py-2.5 md:px-5 md:py-3 lg:px-6 lg:py-3.5 text-[9px] md:text-[10px] font-black uppercase tracking-[0.15em] text-white shadow-xl transition-all duration-300 hover:scale-[1.03] hover:brightness-110 active:scale-97`}>
            Explore Experience <Plus size={12} className="text-white" />
          </a>
          <a href={item.url || "#"} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className={`flex h-9 w-9 md:h-10 md:w-10 lg:h-11 lg:w-11 items-center justify-center rounded-full border border-white/20 bg-[#111218] text-white transition duration-300 hover:bg-white hover:text-black ${theme.borderHover}`}>
          ...
          <Reveal>
            <div className="mx-auto max-w-3xl">
              <div className="mb-5 flex items-center justify-center gap-3">
                ...
                <p className="text-[10px] md:text-[13px] uppercase tracking-[0.45em] text-white/80">Curated Projects</p>
                <span className="h-px w-12 bg-gradient-to-r from-transparent via-amber-400 to-transparent" />
              </div>
              <h2 className="text-[36px] md:text-[72px] font-bold tracking-[-0.04em] leading-none text-white">
                SELECTED <span className="bg-gradient-to-r from-[#fff1c2] via-[#f5c96a] to-[#d89b1d] bg-clip-text text-transparent">WORK</span>
              </h2>
